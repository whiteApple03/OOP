...
class Settings_window_controlling : public Controlling,
                                    public Painter_settings,
                                    public Helper,
                                    public Adapter {
public:
  ...
  void input_keyboard(sf::Event &event) {
    ...
    if (event.type == sf::Event::KeyPressed) {
      ...
      case sf::Keyboard::M:
        ...
        return;
      case sf::Keyboard::Up:
        print_figure(sf::Color::Black, map_config.settings.cur_choise);
        ...
        print_figure(sf::Color::Blue, map_config.settings.cur_choise);
        break;
      ...
      case sf::Keyboard::Enter:
        print_figure(sf::Color::Red, map_config.settings.cur_choise);

        while (map_config.window_settings->waitEvent(event)) {
          if (event.type == sf::Event::KeyPressed) {
            if (event.key.code == sf::Keyboard::Enter) {
              if (*map_config.settings.order[map_config.settings.cur_choise] ==
                  0) {
                continue;
              }
          ...
          if (event.type == sf::Event::TextEntered) {
            if (!sf::Keyboard::isKeyPressed(sf::Keyboard::Enter)) {
              if (new_value.size() < 5 and event.text.unicode >= '0' and
