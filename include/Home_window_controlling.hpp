...
#include "Helper.hpp"
#include "Logic.hpp"
#include "Painter_home_page.hpp"
#include "Settings_renderer.hpp"

class Home_window_controlling : public Controlling,
                                public Helper,
                                public Logic,
                                public Painter_home_page {
public:
  ...
  void auto_change() {
    if (map_config.game_mode and !map_config.input_mode and
        !map_config.settings_mode) {
      ...
      if (map_config.auto_change) {
        if (map_config.cur_time + map_config.delay_between_changed_generations <
            ms) {
  ...
  void map_controlling(sf::Event &event) {
    ...
    else if (!map_config.game_mode and map_config.input_mode and
             !map_config.settings_mode and !map_config.is_resized) {
      if (event.type == sf::Event::KeyPressed) {
        input_keyboard(event);
      }
      if (!map_config.is_resized and
...
private:
  void process_mouse_click() {
    ...
    if (cellX >= map_config.field.sizeX or cellY >= map_config.field.sizeY or
        cellX < 0 or cellY < 0) {
      return;
    }
    ...
    last_clickX = cellX;
    last_clickY = cellY;
    map_config.field.field[cellY][cellX] =
        !map_config.field.field[cellY][cellX];
