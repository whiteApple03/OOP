...
public:
  Settings_renderer() { configurate_settings(); }

  ~Settings_renderer() { delete map_config.window_settings; }
    ...
    while (map_config.window_settings->isOpen()) {
      ...
      if (map_config.window_settings->pollEvent(settings_event)) {
        if (settings_event.type == sf::Event::Resized) {
          sf::FloatRect visiableArea(0, 0, map_config.settings.windowX,
...
private:
  void configurate_settings() {
    ...
    map_config.settings.offsetX =
        map_config.settings.windowX * map_config.margin;
