...
class Painter_settings : public Storage, public Painter {
public:
  void display() {
    ...
    for (int i = 0; i < map_config.settings.property.size() - 1; i++) {
      if (map_config.settings.cur_choise == i) {
        print_figure(sf::Color::Blue, i);
      } else {
        print_figure(sf::Color::Black, i);
      }
      int porperty_size = map_config.settings.property.size();
      property_text.setString(map_config.settings.property[porperty_size - 1]);
      property_text.setPosition(sf::Vector2f(map_config.settings.offsetX,
                                             map_config.settings.offsetY +
                                                 100 * (porperty_size - 1)));
      property_text.setCharacterSize(
          (map_config.settings.windowX /
           map_config.settings.property[porperty_size - 1].size()) *
          2);
  ...
  void print_figure(sf::Color color, int index) {
    float offsetX = map_config.settings.offsetX;
    float offsetY = map_config.settings.offsetY;
    int margin = 100;
    sf::RectangleShape choise(
        sf::Vector2f(map_config.settings.windowX - offsetX, margin));
    sf::Text property_text;
    sf::Text property_value;
    ...
    property_value.setPosition(sf::Vector2f(
        map_config.settings.windowX - offsetX, offsetY + margin * index));
