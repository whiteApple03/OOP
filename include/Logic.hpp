    ...
    for (int i = 0; i < field.sizeY; i++) {
      if (i < field.sizeX - 1) {
        int tmp_sum = ((i >= 1) ? map_config.live_cell_sum[i - 1] : 0) +
                      map_config.live_cell_sum[i] +
                      map_config.live_cell_sum[i + 1];
        if (tmp_sum == 0) {
          continue;
        }
      }
      for (int k = 0; k < field.sizeX; k++) {
        ...
        if (field.field[i][k] and !(counting_live_cells(field, i, k) == 2 or
                                    counting_live_cells(field, i, k) == 3)) {
          cur_coord.first = i;
          cur_coord.second = k;
          changed_cage.push_back(cur_coord);
        } else if (!field.field[i][k] and
                   counting_live_cells(field, i, k) == 3) {
          cur_coord.first = i;
          cur_coord.second = k;
          changed_cage.push_back(cur_coord);
