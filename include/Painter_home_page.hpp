...
class Painter_home_page : public Storage, public Painter {
protected:
  void print_figure(bool is_live, int coord_y, int coord_x) {
    ...
    sf::RectangleShape cell(
        sf::Vector2f(map_config.size_cell, map_config.size_cell));
    // cell.setScale(sf::Vector2f(0, 0));
    cell.setPosition(sf::Vector2f(coord_x * map_config.size_cell + offsetX,
  ...
  void display(std::vector<std::pair<int, int>> mas) {
    for (int i = 0; i < mas.size(); i++) {
      print_figure(map_config.field.field[mas[i].first][mas[i].second],
  ...
  void user_choise(int posX, int posY) {
    sf::RectangleShape cell(
        sf::Vector2f(map_config.size_cell, map_config.size_cell));
    cell.setPosition(
        sf::Vector2f(posX * map_config.size_cell + map_config.offsetX,
...
private:
  ...
  std::vector<std::vector<std::string>> manual = {
      {"Spase - input/game", "w, a, s, d - controlling", "enter - select cell",
       "LMB - change cell state", "k - clear map", "M - open settings"},
      {"Spase - input/game", "o - change generation", "r - auto/manual",
       "k = clear map", "M - open settings"}};
};